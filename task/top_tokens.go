...
func UpdateTokenVolume24h() {
	...
	for _, token := range tokens {
		...
		models.DB.Model(&token).Updates(models.Token{Volume24h: result.Volume24h, Volume24hChange: change})
	}
}
...
func UpdateTokenVolume7d() {
	...
	for _, token := range tokens {
		...
		models.DB.Raw("SELECT sum(volume_usd) as volume7d_last FROM trades where date > ? and date <= ? and (base_token_address = ? or quote_token_address = ?)",
			time14dAgo, time7dAgo, token.Address, token.Address).Select("sum(volume_usd)").Scan(&result)
		...
		models.DB.Model(&token).Updates(models.Token{Volume7d: result.Volume7d, Volume7dChange: change})
	}
}
...
func UpdateTokenVolumeAll() {
	...
	for _, token := range tokens {
		...
		models.DB.Model(&token).Updates(models.Token{VolumeAll: result.VolumeAll})
	}
}
